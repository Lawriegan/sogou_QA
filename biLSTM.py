...
class BiLSTM(object):
    def __init__(self, batch_size, max_sentence_len, embeddings, embedding_size, rnn_size, margin):
        ...
        with tf.variable_scope("LSTM_scope", reuse=None):
            question1 = self.biLSTMCell(questions, self.rnn_size) # shape(?, 100, 200)
            question2 = tf.nn.tanh(self.max_pooling(question1)) # shape(?, 200)
    ...
    @staticmethod
    def max_pooling(lstm_out): # (?, 100, 200)
        height = int(lstm_out.get_shape()[1])
        width = int(lstm_out.get_shape()[2])
        lstm_out = tf.expand_dims(lstm_out, -1) # (?, 100, 200, 1)
        output = tf.nn.max_pool(lstm_out, ksize=[1, height, 1, 1], strides=[1, 1, 1, 1], padding='VALID') # (?, 1, 200, 1)
        output = tf.reshape(output, [-1, width]) # (?, 200)
        return output
